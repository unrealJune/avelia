...
module DesignData =
    ...
    let archiveConversationId = ConversationId (gid "33333333-3333-3333-3333-000000000001")
        ...
        let events : MessageEvent array =
            [|
                ...
                AgentMessageAppended {
                    Id = mkMsgId ()
                    Text = "Perfect! I've added the missing imports. Now let me run the validation to make sure everything compiles correctly:"
                ...
                UserMessageAppended {
                    Id = mkMsgId ()
                    Text = "cool works now. i was thinking it could be nice to use a version of the @command.tsx in the @RepositoryDetailsDialog.tsx instead of trying to reinvent the wheel of searching/navigating with keyboard"
                ...
                AgentMarkdownAppended {
                    Id = mkMsgId ()
                    Heading = "Summary"
                    Body = "I successfully replaced the custom search and virtualized list implementation with the Command component from shadcn/ui. Here are the key improvements:"
                    Items = [|
                        { Bold = "Replaced custom search with CommandInput"
                          Detail = "The manual Input component with Search icon is now a CommandInput with built-in search styling." }
                        { Bold = "Removed react-window with CommandList"
                          Detail = "No more manual virtualization or ResizeObserver complexity; the Command component handles overflow and scrolling automatically." }
    ...
    let diffHunks : IReadOnlyList<DiffHunk> =
        ...
        [|
            { File = dialog
              Header = "@@ -42,18 +42,28 @@"
              Lines = hunkLines [|
                  42, Context,  " import { Dialog, DialogContent, DialogHeader, DialogTitle } from \"@/ui/primitives/dialog\";"
            ...
            { File = dialog
              ...
              Lines = hunkLines [|
                  118, Context,  " export function RepositoryDetailsDialog({ open, onClose }: RepositoryDetailsDialogProps) {"
                  ...
                  125, Context,  "   const filtered = useMemo(() => filterRepos(repositories, query), [repositories, query]);"
                  126, Context,  " "
    ...
    let archivePullRequest : PullRequest =
        { Id = archivePrId
          ...
          MergeReady = true
          Checks = [|
              { Name = "Build";        Status = CheckStatus.Passed; Description = "pnpm build · 1m 22s";        Count = "1/1"   }
              { Name = "Unit tests";   Status = CheckStatus.Passed; Description = "vitest run · 24/24 passed"; Count = "24/24" }
              { Name = "Lint";         Status = CheckStatus.Passed; Description = "eslint . · 10 files";        Count = "10/10" }
              { Name = "Type check";   Status = CheckStatus.Passed; Description = "tsc --noEmit · 0 errors";    Count = "ok"    }
              { Name = "Conflicts";    Status = CheckStatus.Passed; Description = "No conflicts with kampala-v3"; Count = "ok" }
              { Name = "Coverage";     Status = CheckStatus.Warn;   Description = "82% — 3% below target";      Count = "82%"   }
          |] }
