...
| B-11 | 🔄 stores landed | Persistence | **Store interfaces (`IRepositoryStore`/`IWorkspaceStore`/`IConversationStore`/`ISettingsStore`) + `WorkspaceRecord` DTO landed in Abstractions; in-memory impls in `Avelia.Persistence` behind them.** SQLite impls (Microsoft.Data.Sqlite) still pending — they slot in behind the same interfaces with one swap in `RealComposition`. |
