...
**Test count after Chunks 0–5: 149 passing** (54 Avelia.Core, 91 Avelia.Shell.Windows — 9 ThemeService + 11 MainViewModel + 8 WorkspaceVM + 22 WCAG contrast + 11 ThemeUsageLint + 11 PrPaneViewModel + 19 Settings (4 SettingsVM, 7 AppearanceSubpageVM, 4 AgentsSubpageVM, 3 ProfileSubpageVM, + a few overlap with existing), plus 4 from the other F# test projects). Build is clean — 0 warnings, 0 errors.
...
Chunk 6 — PR review page + unified diff viewer. Goal: two-column layout (file tree 280px / `*` diff pane), per-file diff hunks rendered with +/− coloring, header card with file path + diff stats + Unified/Split SegmentedControl, "All checks passed" InfoBar, footer action row (Request changes / Comment / Approve). `PrPaneViewModel.FileOpened` already raises a typed `RelativePath` event — the chunk's first hook is `MainWindow` (or `WorkspacePage` parent) catching that event and calling `ContentFrame.Navigate(typeof(PrReviewPage), …)` with the file pre-selected. `IDiffService.GetHunksAsync` is already wired against `DesignData.diffHunks`. See "Chunk 6" section below.
...
- Backlog item for Settings: density/transparency toggles fire setters but no consumer applies them visually yet. Density is a no-op until row paddings / chat spacing are conditionally bound; Transparency toggle should swap `SystemBackdrop` between `MicaBackdrop` and a solid surface brush. Both queued for Chunk 9 polish.
