...
Chunk 8 — Add Repository ContentDialog. Goal: WinUI 3 `ContentDialog` with three tabs (Local folder / Git URL / Clone from GitHub), per-tab content (recent repos under Local; SSH-detected InfoBar under Git URL; GitHub search under Clone). `IRepositoryService.AddAsync` is the existing F# contract; new wiring is shell-side dialog + view-model + `FolderPicker`. Title-bar `+` button and rail "Add repository" both open the dialog; adding closes it and the new repo appears in the rail tree. See "Chunk 8" section below.
