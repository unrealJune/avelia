...
| 7 — Inbox page | ✅ done | `InboxPage` (Grid Auto/* with header + filled card hosting a ListView) + `InboxItemTemplateSelector` (3 per-kind row templates so leading tile brushes stay inline `{ThemeResource}` — same pattern Chunk 6 used for diff lines). `InboxItemRowBody` UserControl carries the shared row body (title / description / time / chevron) so the per-kind templates only own the tile (background brush + glyph + glyph foreground). `InboxViewModel` loads from `IInboxService.ListAsync`, projects each `InboxItem` into an `InboxItemViewModel`, computes the "N unread · N total" header (every seeded item treated as unread until persistence tracks read state), and owns the `OpenCommand` that publishes `WorkspaceOpenRequested` for the rail to translate into `MainViewModel.OpenWorkspace`. New `AveliaInfoBgBrush` token (Warning/Success already had a bg twin; Info did not). In-flight CTS guard mirrors `PrReviewViewModel.LoadAsync` for last-load-wins under fast renavigation. Sentinel `WorkspaceId.NewWorkspaceId(Guid.Empty)` gates rows without a linked workspace — clicking those silently no-ops rather than firing an open. |
| Post-Chunk-7 review fixes | ✅ done | Code-review pass against the Chunk-7 surface — bug + quality fixes in the "Chunk-7 review fixes" section below. 133 shell + 54 core tests green. |
...
| Post-Chunk-8 review fixes | ✅ done | Code-review pass against the Chunk-8 surface — bug + quality fixes in the "Chunk-8 review fixes" section below. 135 shell + 55 core tests green. |
| 9 — Accessibility & test pass | ⏳ pending | |
| 10 — Real backend (Persistence/VCS/Agent) | ⏳ future | Out of scope for v1 |

**Test count after Chunk 7 review: 191 passing** (54 Avelia.Core, 133 Avelia.Shell.Windows — Chunk-7's 7 `InboxViewModelTests` plus 4 new diff-tile WCAG pairings (Warning/Info × Light/Default) on the prior 122 shell tests, plus 4 from the other F# test projects). Build is clean — 0 warnings, 0 errors.

### Chunk-7 review fixes

Ran the WinUI code-review skill against the Chunk-7 surface. Bundled fixes:

| ID | What | File(s) |
|---|---|---|
| **N-1** Exhaustive dispatch on `InboxItemKind` | `InboxItemTemplateSelector` used a cascade of `row.Kind.IsWarning` / `IsSuccess` checks with `InfoTemplate` as the default. A future fourth `InboxItemKind` case would silently render with the Info template instead of failing the build. Added `InboxItemKind.Match` on the F# DU (mirrors `MessageEvent.Match` / `ModelChoice.Match` precedent) and migrated the selector to dispatch through it — adding a new case is now a compile error at the selector. | `src/Avelia.Core.Abstractions/DomainTypes.fs`, `Controls/InboxItemTemplateSelector.cs` |
| **N-2** Triple-template duplication | The three per-kind row `DataTemplate`s were ~135 lines of near-identical XAML differing only in the leading tile (background brush + glyph + glyph foreground). Extracted the shared body (title / description / time / chevron) into a new `InboxItemRowBody` UserControl with an `Item: InboxItemViewModel` DP. Per-kind templates shrunk to ~15 lines each; layout tweaks are now a one-file change. | `Pages/InboxPage.xaml`, `Controls/InboxItemRowBody.xaml{,.cs}` |
| **N-3** Nested ScrollViewer + ListView UX jank | Outer `ScrollViewer` wrapping a `StackPanel` + inner `ListView.ScrollViewer.VerticalScrollBarVisibility="Auto"` yielded two scrollbars on a long inbox. Restructured the page as a `Grid (Auto, *)` matching the `PrReviewPage` pattern: header in row 0, card in row 1 fills remaining height, and the ListView scrolls inside the card. One scrollbar, virtualization preserved. | `Pages/InboxPage.xaml` |
| **N-4** Open-workspace flash with async backends | `NavigateToInbox`'s closure called `OpenWorkspaceCommand.Execute(id)` (fire-and-forget) immediately followed by `RailNav.SelectedItem = HomeItem`. With stub services every `Execute` runs synchronously; with a real backend's async `IWorkspaceService.GetAsync`, the rail flip would land first and `NavigateToActiveWorkspace` would briefly render the previous workspace until `ActiveTab` settled. Changed `InboxPageArgs.OnWorkspaceOpenRequested` to `Func<WorkspaceId, Task>`, the closure to `async id => { await OpenWorkspaceCommand.ExecuteAsync(id); RailNav.SelectedItem = HomeItem; }`, and the page's bridge to `async void` with a try/catch so an unobserved task can't crash the process. No observable behavior change on stubs; future real backends won't flash. | `MainWindow.xaml.cs`, `Pages/InboxPage.xaml.cs` |
| **N-5** Inbox tile contrast pairings missing from `ThemeContrastTests` | `ThemeContrastTests` table covered `AveliaSuccessBrush`/`SuccessBgBrush` and `AveliaDangerBrush`/`DangerBgBrush` but not `AveliaWarningBrush`/`WarningBgBrush` (used by the warning tile) or the new `AveliaInfoBrush`/`InfoBgBrush` (added for the info tile). A future redesign that darkened either bg brush would let the glyph blend without tripping the lint. Added two pairings × two themes; all four pass WCAG AA large-text. | `tests/Avelia.Shell.Windows.Tests/ThemeContrastTests.cs` |

### Chunk-7 design notes

| Decision | Why | Where |
|---|---|---|
| **Three inbox-row `DataTemplate`s** (Warning / Success / Info) selected by `InboxItemTemplateSelector` rather than one row with a brush converter. | A converter that returned a `SolidColorBrush` based on `InboxItemKind` would resolve the brush once and freeze it — the exact regression class `ThemeUsageLintTests` guards (and the E-1 dark-mode bug from Chunk 3). The template-per-kind pattern keeps the tile background + glyph foreground as inline `{ThemeResource}` references that re-resolve on theme switch. Same pattern Chunk 6 used for diff lines. | `Pages/InboxPage.xaml`, `Controls/InboxItemTemplateSelector.cs` |
| **Page-owned `OpenCommand`, not per-row commands** | One publisher for `WorkspaceOpenRequested` keeps the shell's subscription point single and obvious (the `NavigateToInbox` closure routes the id into `MainViewModel.OpenWorkspace`). Per-row `RelayCommand` would let the row VM know about the workspace-opening flow, which it has no reason to. | `ViewModels/InboxViewModel.cs`, `Pages/InboxPage.xaml.cs` |
| **`HasLinkedWorkspace` derived in the row VM, sentinel `Guid.Empty`** | `InboxItem.LinkedWorkspaceId` is typed `WorkspaceId` but `DomainTypes.fs:351` documents `Guid.Empty` as the "unset" convention. The VM compares against `WorkspaceId.NewWorkspaceId(Guid.Empty)` (F#-generated factory — single-case DUs with public visibility don't expose a public C# constructor). Rows without a linked workspace silently no-op on click. | `ViewModels/InboxItemViewModel.cs` |
| **`InboxPageArgs.OnWorkspaceOpenRequested` callback, not a back-pointer to `MainViewModel`** | Same shape as `SettingsPageArgs.BackAction`. The page never sees the root VM; the shell decides what "open this workspace" means (today: `OpenWorkspaceCommand.Execute(id)` + flip rail selection back to Home). | `Pages/InboxPage.xaml.cs`, `MainWindow.xaml.cs` |
| **`UnreadCount = TotalCount` for now** | The F# `InboxItem` record has no `IsRead` field. Adding one to satisfy a header subtitle that the design shows would be speculative — wait until the real backend (Chunk 10) needs to track read state, then add the field and update the projection. | `ViewModels/InboxViewModel.cs` |
| **No `InboxItemRow` UserControl** | The plan named one. Inline `DataTemplate`s in the page work because we only have three kinds and the row markup is short — a UserControl-per-row would either duplicate the same per-kind split into 3 controls or push the brush switching into code-behind (re-introducing the freezing-brush problem). Inline templates plus the selector keep the count at one resource per kind without scattering files. | `Pages/InboxPage.xaml` |
...
Chunk 9 — Accessibility, automation, polish, test pass. Goal: close the seven UX surfaces in CLAUDE.md §UX testing requirements. With Chunks 0–8 all landed, the remaining backlog (density/transparency toggles, dead PR-review affordances, View-file button, `x:Uid` migration, axe-windows runtime scan, E2E flows) collapses into one chunk.
...
Notes left behind from Chunk 7:
- `InboxItem` has no `IsRead` field yet — the page derives `UnreadCount = TotalCount` from the inbox count. When persistence (Chunk 10) adds the field, the `InboxViewModel.LoadAsync` projection becomes `inbox.Count(i => !i.IsRead)` and the header subtitle stays the same shape. No XAML changes needed.
- All 4 seeded `InboxItem` rows have non-empty `LinkedWorkspaceId`s, so the `HasLinkedWorkspace = false` branch in `OpenCommand` is currently exercised only by tests. The page renders the chevron unconditionally; a future visual tweak could dim/hide it on unlinked rows, but the design doesn't currently call for it.
