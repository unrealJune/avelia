...
let private initRepo () =
    let root = Path.Combine(Path.GetTempPath(), "avelia-svc-" + Guid.NewGuid().ToString("N"))
...
let private cleanup (path: string) =
    try
        for f in Directory.EnumerateFiles(path, "*", SearchOption.AllDirectories) do
            try File.SetAttributes(f, FileAttributes.Normal) with _ -> ()
...
let ``CreateAsync materializes a real worktree on disk`` () =
    ...
    let worktreesRoot = Path.Combine(Path.GetTempPath(), "avelia-wt-" + Guid.NewGuid().ToString("N"))
    ...
    try
        ...
        | Success ws ->
            ...
            Assert.True(File.Exists(Path.Combine(record.WorktreePath.Value, "README.md")), "worktree should contain the repo's tracked files")
