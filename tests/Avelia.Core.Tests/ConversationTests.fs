...
let ``Replay produces conversation whose LastSequence equals event count``
    (events: MessageEvent list)
    =
    let c =
        Conversation.replay
            (ConversationId.create ())
            ...
            "test"
            events
    c.LastSequence = events.Length
    && c.Messages.Length = events.Length

[<Property(Arbitrary = [| typeof<Generators.Arbs> |])>]
let ``Replay preserves event order``
    (events: MessageEvent list)
    =
    let c =
        Conversation.replay
...
[<Property(Arbitrary = [| typeof<Generators.Arbs> |])>]
let ``applyEvent then replay subset matches incremental fold``
    (events: MessageEvent list)
    =
    ...
    let viaFold = events |> List.fold Conversation.applyEvent (Conversation.empty id wsId title)
    viaReplay = viaFold
